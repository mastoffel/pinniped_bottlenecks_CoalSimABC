...
all_seals_full <- sealABC::read_excel_sheets("../data/seal_data_largest_clust_and_pop.xlsx")[1] # [1:28]
...
all_sumstats_full <- lapply(all_seals_full, function(x) mssumstats(x, datatype = "microsats", 
                                                                   start_geno = 4, mratio = "loose"))
...
sumstats <- c("num_alleles_mean", "num_alleles_sd",
              ...
              "exp_het_mean", "mean_allele_range", "mratio_mean") # , "mratio_mean"
...
all_fits_bot <- apply(all_sumstats, 1, abc::gfit, sumstat = sims_stats, nb.replicate = cv_rep, tol = tol, subset = models == "bot")
all_fits_neut <- apply(all_sumstats,  1, abc::gfit, sumstat = sims_stats, nb.replicate = cv_rep, tol = tol, subset = models == "neut")
